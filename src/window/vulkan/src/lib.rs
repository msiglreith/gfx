...
// TODO: dynamically set extensions depending on the platform
...
pub fn init<T: gfx_core::format::RenderFormat>(title: &str, requested_width: u32, requested_height: u32)
...
#[cfg(target_os = "windows")]
fn surface_windows(backend: gfx_device_vulkan::SharePointer, hwnd: *mut ()) -> vk::SurfaceKHR {
    let (inst, vk) = backend.get_instance();
...
#[cfg(all(unix))]
fn surface_xcb(backend: gfx_device_vulkan::SharePointer, conn: *mut (), window: *mut ()) -> vk::SurfaceKHR {
    let (inst, vk) = backend.get_instance();
    let info = vk::XcbSurfaceCreateInfoKHR {
        sType: vk::STRUCTURE_TYPE_XCB_SURFACE_CREATE_INFO_KHR,
        pNext: ptr::null(),
        flags: 0,
        connection: conn as *mut _,
