...
use core::{buffer, command, device as d, image as i, memory as m, pass, pool, pso, state};
use core::{
...
fn main() {
    ...
    let set_layout = device.create_descriptor_set_layout(&[
            pso::DescriptorSetLayoutBinding {
                binding: 0,
                ty: pso::DescriptorType::ConstantBuffer,
                count: 1,
    ...
    let (locals_buffer, locals_memory) = {
        ...
        let buffer_unbound = device.create_buffer(buffer_len, buffer_stride, buffer::CONSTANT).unwrap();
